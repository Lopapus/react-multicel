...
import ModeloSchema from './schemas/ModeloSchema'
...
const FormModelos = () => {
  ...
  const handleFindModelo = async () => {
    try {
      if (params.id === 'crear' || params.id == null) {
        ...
        setLoading(true)
        const response = await fetchToken(`${Server}/modelos/${params.id}`)
        setLoading(false)
  ...
  const handleSubmitForm = async (e) => {
    e.preventDefault()
    try {
      await ModeloSchema.validate(form, { abortEarly: false })
      handleUploadModelo()
    } catch (errors) {
      if (errors?.inner) {
  ...
  return (
    data
        ...
        <div className="card-body">
          <form onSubmit={handleSubmitForm}>
            ...
            <div className='form-group my-3 d-flex justify-content-center justify-content-sm-start'>
              <ButtonIcon type="button" btncolor='btn-secondary me-1' iconclass={'fas fa-arrow-left'} handler={() => navigate('/parametros/modelos')} >
                Volver
              </ButtonIcon>
      ...
      : loading ? <Loader /> : <Alert variant="danger" className="text-center">No se encontró el modelo</Alert>
  )
}

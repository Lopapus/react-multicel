...
import TipoOfertaSchema from './schemas/TipoOfertaSchema'
...
const FormOfertas = () => {
  ...
  const handleFindOferta = async () => {
    try {
      if (params.id === 'crear' || params.id == null) {
        ...
        setLoading(true)
        const response = await fetchToken(`${Server}/tipos_ofertas/${params.id}`)
        setLoading(false)
  ...
  const handleUploadTipoOferta = async () => {
  ...
  const handleSubmitForm = async (e) => {
    e.preventDefault()
    try {
      await TipoOfertaSchema.validate(form, { abortEarly: false })
      handleUploadTipoOferta()
    } catch (errors) {
      if (errors?.inner) {
    ...
    data
      ? <div className="card border-0 shadow mt-3">
        <div className="card-header border-bottom d-flex align-items-center justify-content-between">
          <h2 className="fs-5 fw-bold mb-0">Tipos de ofertas</h2>
        </div>
        ...
        <div className="card-body">
          <form onSubmit={handleSubmitForm}>
            ...
            <div className='form-group my-3 d-flex justify-content-center justify-content-sm-start'>
              <ButtonIcon type="button" btncolor='btn-secondary me-1' iconclass={'fas fa-arrow-left'} handler={() => navigate('/parametros/ofertas')} >
                Volver
              </ButtonIcon>
